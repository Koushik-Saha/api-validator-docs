  ...
  "src/core/Validator.ts"() {
    ...
    Validator = class {
      ...
      validateRequest(method, path, data) {
        ...
        try {
          if (endpoint.params) {
            const paramSchema = import_zod.z.object(endpoint.params);
            paramSchema.parse({});
          }
          if (endpoint.query && data?.query) {
            const querySchema = import_zod.z.object(endpoint.query);
            querySchema.parse(data.query);
          }
          if (endpoint.body && data?.body) {
            const bodySchema = import_zod.z.object(endpoint.body);
      ...
      validateResponse(method, path, responseData) {
        ...
        try {
          const responseSchema = import_zod.z.object(endpoint.response);
...
__export(index_exports, {
  ...
  Validator: () => Validator,
  createAPIValidator: () => createAPIValidator,
